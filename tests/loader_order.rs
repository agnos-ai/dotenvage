...
fn clear_env(keys: &[&str]) {
    for k in keys { unsafe { env::remove_var(k) }; }
}

#[test]
#[serial]
fn order_no_env_uses_local() {
    clear_env(&["DOTENVAGE_ENV","EKG_ENV","DOTENVAGE_ARCH","EKG_ARCH","DOTENVAGE_USER","EKG_USER","GITHUB_EVENT_NAME","GITHUB_REF","PR_NUMBER","USER","USERNAME"]);
...
fn order_env_prod() {
    clear_env(&["DOTENVAGE_ENV","EKG_ENV","DOTENVAGE_ARCH","EKG_ARCH","DOTENVAGE_USER","EKG_USER","GITHUB_EVENT_NAME","GITHUB_REF","PR_NUMBER","USER","USERNAME"]);
    unsafe { env::set_var("DOTENVAGE_ENV","prod"); }
    let tmp = tempfile::tempdir().unwrap();
    std::fs::write(tmp.path().join(".env"), "").unwrap();
...
fn order_env_arch() {
    clear_env(&["DOTENVAGE_ENV","EKG_ENV","DOTENVAGE_ARCH","EKG_ARCH","DOTENVAGE_USER","EKG_USER","GITHUB_EVENT_NAME","GITHUB_REF","PR_NUMBER","USER","USERNAME"]);
    unsafe { env::set_var("EKG_ENV","prod"); }
    unsafe { env::set_var("DOTENVAGE_ARCH","docker-s3"); }
    let tmp = tempfile::tempdir().unwrap();
    std::fs::write(tmp.path().join(".env"), "").unwrap();
    std::fs::write(tmp.path().join(".env.prod"), "").unwrap();
...
fn order_user_and_arch() {
    ...
    unsafe { env::set_var("DOTENVAGE_ENV","prod"); }
    unsafe { env::set_var("DOTENVAGE_ARCH","docker-s3"); }
    unsafe { env::set_var("DOTENVAGE_USER","alice"); }
    let tmp = tempfile::tempdir().unwrap();
    std::fs::write(tmp.path().join(".env"), "").unwrap();
    std::fs::write(tmp.path().join(".env.prod"), "").unwrap();
    ...
    let loader = dotenvage::EnvLoader::with_manager(dotenvage::SecretManager::generate().unwrap());
    let got = names(loader.resolve_env_paths(tmp.path()));
    assert_eq!(got, vec![".env", ".env.prod", ".env.prod-alice", ".env.prod-docker-s3-alice"]);
}
...
fn order_github_pr_number() {
    clear_env(&["DOTENVAGE_ENV","EKG_ENV","DOTENVAGE_ARCH","EKG_ARCH","DOTENVAGE_USER","EKG_USER","GITHUB_EVENT_NAME","GITHUB_REF","PR_NUMBER","USER","USERNAME"]);
    unsafe { env::set_var("GITHUB_EVENT_NAME","pull_request"); }
    unsafe { env::set_var("PR_NUMBER","123"); }
    let tmp = tempfile::tempdir().unwrap();
    std::fs::write(tmp.path().join(".env"), "").unwrap();
...
fn order_github_ref_parsing() {
    clear_env(&["DOTENVAGE_ENV","EKG_ENV","DOTENVAGE_ARCH","EKG_ARCH","DOTENVAGE_USER","EKG_USER","GITHUB_EVENT_NAME","GITHUB_REF","PR_NUMBER","USER","USERNAME"]);
    unsafe { env::set_var("GITHUB_REF","refs/pull/456/merge"); }
    let tmp = tempfile::tempdir().unwrap();
    std::fs::write(tmp.path().join(".env"), "").unwrap();
